...
namespace iv4 {
  ...
  SocketInterface::SocketInterface(IV4MessageCallback callback, bool server, const std::string &name) :
        ...
        if(server) {
            ...
            if (bind(serverSocket, (const struct sockaddr *) &server_address, address_length) < 0) {
              ...
              debug << Debug::Mode::Failure << "Failed to bind socket: " << strerror(errno) << std::endl;
                close(serverSocket);
                serverSocket = -1;
                return;
            }
            ...
            if(listen(serverSocket, 2) < 0) {
              ...
              debug << Debug::Mode::Failure << "Failed to listen on socket: " << strerror(errno) << std::endl;
                close(serverSocket);
                serverSocket = -1;
                return;
            }

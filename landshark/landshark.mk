...
$(call inherit-product, device/xilinx/landshark/device-common.mk)
$(call inherit-product, device/xilinx/landshark/landshark/device-landshark.mk)

# Inherit full base product
$(call inherit-product, $(SRC_TARGET_DIR)/product/full_base.mk)

# Or try to split them up here?
# $(call inherit-product, device/xilinx/landshark/iveia_base.mk)

# Include vendor binaries
$(call inherit-product-if-exists, vendor/xilinx/zynqmp/device-vendor.mk)

# Specify product details
PRODUCT_NAME := landshark
PRODUCT_DEVICE := landshark
PRODUCT_BRAND := Android
PRODUCT_MODEL := AOSP on ZynqMP iVeia landshark carrier
PRODUCT_MANUFACTURER := Xilinx

# Specify variables for kernel build
KERNEL_SRC_DIR ?= $(ANDROID_BUILD_TOP)/linux-iveia
KERNEL_CFG_NAME ?= iveia_atlas_ii_z8_landshark_defconfig
KERNEL_DTS_NAMES ?= iveia-landshark.dts
